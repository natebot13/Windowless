...
class Window(pyglet.window.Window):
    ...
    def __init__(self, id: int, effect, fcn: str, game, *args, **kwargs):
    ...
    def move(self, x: float, y: float):
        l = self.fcn(x, y, self, self.game)
        if l and l != self.get_location():
            if -self.width < l[0] < self.game.res[0] and -self.height < l[1] < self.game.res[1] or self.game.offset == [0, 0]:
    ...
    def on_draw(self):
        ...
        self.game.deer.sprite.position = [util.x_to_window(self.game.deer.x, self, self.game.offset) - self.game.deer.sprite.width * (self.game.deer.sprite.scale_x-1)/2,
                                          util.y_to_window(self.game.deer.y + self.game.deer.sprite.height * (self.game.deer.sprite.scale_y * .5 - .5), self, self.game.offset)]
        self.game.BG.blit(util.x_to_window(0, self, self.game.offset), util.y_to_window(self.game.res[1], self, self.game.offset))
        self.game.deer.sprite.draw()
        for p in self.game.platforms:
            pyglet.graphics.draw_indexed(4, pyglet.gl.GL_TRIANGLES, [0, 1, 2, 0, 2, 3],
                                         ('v2i', util.points_to_window([p.x1, p.y2, p.x1, p.y1, p.x2, p.y1, p.x2, p.y2], self, self.game.offset)),
                                         ('c4B', (p.c * 4)))
...
movements = {
    ...
    'half circle': lambda x, y, w, game: (math.sin((abs(game.time*3 % 360-180)-90)*math.pi/180)*200+4640-game.offset[0],
                                          math.cos((abs(game.time*3 % 360-180)-90)*math.pi/180)*200+400-game.offset[1])
}
